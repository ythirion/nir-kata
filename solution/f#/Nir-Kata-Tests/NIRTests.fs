...
open Nir_Kata.Primitive.Obsession
open Xunit
...
let ``Validate Nir should return false`` (input: string, reason: string) =
    Assert.False(NIR.validate input, reason)
...
let ``Validate Nir should return true`` (input: string) = Assert.True(NIR.validate input)
